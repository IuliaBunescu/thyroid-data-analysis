...
tsh = st.sidebar.number_input("TSH Level (mIU/L)", 0.0, 10.0, 2.5, 0.1, disabled=True)
t3 = st.sidebar.number_input("T3 Level (ng/dL)", 0.0, 300.0, 120.0, 1.0, disabled=True)
t4 = st.sidebar.number_input("T4 Level (μg/dL)", 0.0, 20.0, 8.0, 0.1, disabled=True)
ft4 = st.sidebar.number_input("Free T4 (ng/dL)", 0.0, 3.0, 1.2, 0.1, disabled=True)

# st.sidebar.subheader("Analysis Options")
show_reference_range = st.sidebar.checkbox("Show Reference Range", value=True)
data_points = st.sidebar.slider(
    "Number of Sample Data Points", 10, 100, 50, disabled=True
)
...
with tab1:
    ...
    with col3:
        st.metric(
            "Male/Female Ratio",
            f"{(sample_data['Gender'] == 'Male').sum()}/{(sample_data['Gender'] == 'Female').sum()}",
        )
    with col4:
    ...
    with col1:
        ...
        if show_reference_range:
            fig1.add_vline(
                ...
                line_dash="dash",
                line_color="green",
                annotation_text="Min Normal",
            )
            fig1.add_vline(
                x=4.5, line_dash="dash", line_color="red", annotation_text="Max Normal"
            )
...
with tab3:
    ...
    if tsh < 0.5:
        issues.append(
            "⚠️ TSH level is below normal range - may indicate hyperthyroidism"
        )
    elif tsh > 4.5:
